...
- Someting to clarify:
  - I have already refine the pretrained ViT, and now it perform a lot better than what is shown in presentaion slides and it grow steadily (I warm up and then let lr go down steadly and finally linear slight adjust, it is shown in the schedule)
  - We find out a lot of problem and did many experiments to solve them, such as data leakage when additional data in Oxford102 has overlap with Oxford17
    - We will just give the final non-data leakage data and code but not the one with this issue. 
  - In the freezing pretrain-ViT part, this version make it partially frozon because it got best performance
